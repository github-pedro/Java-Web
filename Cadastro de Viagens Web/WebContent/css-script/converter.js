...
function converter() {
	...
	// BUSCA TODOS AS COTAÇÕES DAS MOEDAS NO JSON
  ...
  function getAtualizacao(moeda) {
	...
	// BUSCA A ÚLTIMA ATUALIZAÇÃO DA COTAÇÃO NO DIA
    ...
    atualizacao.innerHTML = 'Cotação atualizada em ' + dataFormatada;
 	...
 	// BUSCA OS PARÂMETROS DIGITADO/SELECIONADO PELO USUÁRIO
  ...
  var selecionado = document.querySelector("#moedas").value;
	...
	// FAZ A VERIFICAÇÃO CASO O USUÁRIO NÃO PREENCHA ALGUM CAMPO REQUERIDO
  ...
  if (num <= 0) {
	...
	// COMEÇO DO CALCULO APÓS OS PARÂMETROS SEREM PREENCHIDOS

  if (selecionado == "EUR" && isNaN(num) == false) {
	
	// FAZ O CALCULO DA COTAÇÃO DO DIA ENTRE EUR E BRL
    calculo = num * euro

	// CONVERTE A VARIAVEL PARA VALOR MONETÁRIO PARA APRESENTAR NA TELA EX: R$ 2,00
    num = num.toLocaleString('en-us', { style: 'currency', currency: 'EUR' });
    calculo = calculo.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' });

	// MANDA OS RESULTADOS PARA A TELA DO USUÁRIO
